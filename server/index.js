...
io.on("connection", (socket) => {
  ...
  socket.on("playersList", (info) => {
    if (info && rooms[info.partida]) {
      io.to(info.partida).emit(
        "getPlayersList",
        Object.values(rooms[info.partida].players)
      );
    }
  });
  ...
  socket.on("getQuestionsList", (info) => {
    if (info) {
      ...
      io.to(info.partida).emit("getQuestionsList", {
        questionsList,
        numRandom,
      });
    }
  ...
  socket.on("saveResp", (info) => {
    if (info) {
      ...
      if (!rooms[partida].answers[preguntaId])
        rooms[partida].answers[preguntaId] = new Set();
  ...
  socket.on("saveLastResp", (info) => {
    if (info) {
      rooms[info.partida].players[socket.id] = {
        ...rooms[info.partida].players[socket.id],
        ultimaRespuesta: info.respuesta.respuesta,
      };
    }
  });
